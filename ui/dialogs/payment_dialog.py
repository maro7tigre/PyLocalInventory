...
def _ensure_payments_table(database):
    if not database or not database.cursor:
        raise RuntimeError("No database connection is available for payments.")
...
class PaymentDialog(QDialog):
    ...
    def __init__(self, sale_obj, database, parent=None, config=None):
        ...
        self.currency = self.config.get('currency') or 'MAD'
    ...
    def _setup_ui(self):
        ...
        # Domain totals are Decimal; Qt spin boxes use float. Normalize once so
        # subtraction and formatting below never mix Decimal and float.
        self._total = float(self.sale_obj.calculate_total_price() or 0)
